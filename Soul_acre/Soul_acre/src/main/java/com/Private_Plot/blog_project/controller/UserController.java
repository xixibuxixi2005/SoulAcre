...
public class UserController {
    ...
    public ResponseEntity<Map<String, String>> registerUser(@RequestBody User user, HttpServletResponse response) {
        ...
        if(user.getUsername().length() < 4 || user.getUsername().length()>20||
            ...
            responseBody.put("error", "请勿修改前端代码（用户名或密码长度违规）");
            return new ResponseEntity<>(responseBody, HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity<Map<String, String>> changePassword(@RequestBody Map<String, String> request, HttpServletResponse response) {
        ...
        if (user == null) {
            responseBody.put("error", "用户不存在");
            return new ResponseEntity<>(responseBody, HttpStatus.NOT_FOUND);
        }

        if (!user.getPassword().equals(oldPassword)) {
            responseBody.put("error", "旧密码错误");
            return new ResponseEntity<>(responseBody, HttpStatus.UNAUTHORIZED);
        }

        if (newPassword.length() < 4 || newPassword.length() > 20) {
            responseBody.put("error", "新密码长度必须在 4 到 20 个字符之间");
            return new ResponseEntity<>(responseBody, HttpStatus.BAD_REQUEST);
        }
        ...
        responseBody.put("success", "密码修改成功");
        responseBody.put("token", token);
        return new ResponseEntity<>(responseBody, HttpStatus.OK);
    }
